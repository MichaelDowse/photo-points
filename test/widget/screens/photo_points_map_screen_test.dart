...
void main() {
  group('PhotoPointsMapScreen Widget Tests', () {
      ...
      return MaterialApp(
        ...
        routes: {
          '/photo_point_detail': (context) => Scaffold(
            body: Text('Photo Point Detail: ${ModalRoute.of(context)!.settings.arguments}'),
          ),
        },
      );
    }

    testWidgets('renders correctly with all required components', (WidgetTester tester) async {
    ...
    testWidgets('shows empty state when no photo points have coordinates', (WidgetTester tester) async {
      ...
      expect(find.text('Photo points without GPS coordinates will not appear on the map.'), findsOneWidget);
      expect(find.byIcon(Icons.location_off), findsOneWidget);
    });

    testWidgets('shows map when photo points have coordinates', (WidgetTester tester) async {
      final photoPointsWithCoordinates = TestData.createMockPhotoPointList(count: 2);
    ...
    testWidgets('shows map with mixed photo points (some with coordinates, some without)', (WidgetTester tester) async {
    ...
    testWidgets('tapping retry button clears error and reinitializes', (WidgetTester tester) async {
    ...
    testWidgets('floating action button has correct tooltip and icon', (WidgetTester tester) async {
      await tester.pumpWidget(createTestWidget(child: const PhotoPointsMapScreen()));
      await tester.pump();
    ...
    testWidgets('my location button has correct tooltip', (WidgetTester tester) async {
      await tester.pumpWidget(createTestWidget(child: const PhotoPointsMapScreen()));
      await tester.pump();
    ...
    testWidgets('shows all required map components when rendering map', (WidgetTester tester) async {
      final photoPointsWithCoordinates = TestData.createMockPhotoPointList(count: 1);
      mockAppStateProvider.setPhotoPoints(photoPointsWithCoordinates);
    ...
    testWidgets('handles consumer properly for app state changes', (WidgetTester tester) async {
      await tester.pumpWidget(createTestWidget(child: const PhotoPointsMapScreen()));
      await tester.pump();
